...
export const leerPersonasNaturales_SR = async (search) => {
  try {
    const result = await pool.query("SELECT * FROM leer_personas_natural($1)", [
      search,
    ]);
    return {
      status: "success",
...
export const crearPersonaNatural_SC = async (
  nombre,
  direccion,
  fecha_registro,
  identificacion,
...
export const actualizarPersonaNatural_SU = async (
  codigo,
  nombre,
  direccion,
  fecha_registro,
