...
async function fetchData() {
  try {
    ...
    const response = await fetch(
      `${apiUrl}?search=${searchTerm}&page=${currentPage}&limit=${limit}`
    );
...
function renderTable() {
  ...
  const canCreate = privileges.includes("modelo_avion_create");
  const canUpdate = privileges.includes("modelo_avion_update");
  const canDelete = privileges.includes("modelo_avion_delete");
...
document.querySelectorAll("th.sortable").forEach((th) => {
  th.addEventListener("click", () => {
    ...
    document
      ...
      .forEach((th) => th.classList.remove("desc"));
